...
## 2. Running I-ETL (create a database of your data)
...
3. The absolute path to your MongoDB folder in `SERVER_FOLDER_MONGODB` (this can be any folder on your machine; it will contain MongoDB's data)
4. The locale to be used to read your data in `USE_LOCALE` (this is imoprtant to read dates and numeric values with your country's norm)
5. _If you want to keep the execution log: the absolute path to your log folder in `SERVER_FOLDER_LOG_ETL` (this can be any folder on your machine; it will contain the ETL's log files)_
   

### Scenario 1: run the ETL with real data

1. We expect that:
  - **Variables are columns, patients are rows** and patient have identifiers (which will be further anonoymized by I-ETL), so please pre-process your data if this is not the case.
  - The input files have the **exact same name as specififed in the metadata** (https://drive.google.com/drive/u/0/folders/1eJOtoXj192Z9u0VENn4BdOfJ2wm5xZ5Z)

2. Set also the following parameters in the `.env` file: 
   - The absolute path to the folder containing your metadata file in `SERVER_FOLDER_METADATA`.
   - The name of the file containing the metadata in `METADATA`.
   - The absolute path to the folder containing your data in `SERVER_FOLDER_DATA`.
   - The list of file names containing you data, separated by commas (no space) in `DATA_FILES`.
   - Set the column name containing your patient IDs (e.g., `PatientID`, or `pid`, etc) in `PATIENT_ID`.
...
3. Run `docker compose --env-file ${ABSOLUTE_PATH_ENV_FILE} up -d`
...
| `NB_ROWS`                       | The number of generated patients.                                                  | 100 or any other integer    |
...
| `SERVER_FOLDER_DATA`* | The absolute (server) path to the folder containing the datasets.                | `/dev/null` or a folder path |
...
| `SAMPLE_ID` | The name of the column in the data containing sample IDs | `` (empty), empty if you do not have sample data, else a column name |
...
6. Move to `/somewhere/in/host/machine/` using `cd /somewhere/in/host/machine`
7. Load the TAR image within the Docker of the host machine: `docker load < the-ietl-image.tar`
8. Follow any above scenario, i.e., tune the .env file and run I-ETL
