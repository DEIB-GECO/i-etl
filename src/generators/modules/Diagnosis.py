...
SJD_df_diseases = pd.read_csv(get_ground_data("SJD_Diseases.csv"), sep=",")


# df_diseases = pd.read_csv(os.path.join(GROUND_DATA_FOR_GENERATION, "IMGGE_Diseases.csv"), sep=",")
...
class Diagnosis:
    ...
    def get_symptoms_by_omim_id(omim_id=int, symptoms_max_number=int):
        if omim_id is not None:
            ...
            x = df_genes_to_phenotype.loc[
                df_genes_to_phenotype['disease_id'] == omim_id_list[0]]  # We compare with the first OMIM ID
            ...
            for i, row in x.iterrows():
                ...
                if hpo not in ["HP:0000007", "HP:0000006", "HP:0001417", "HP:0001419", "HP:0001423", "HP:0001426",
                               "HP:0001427", "HP:0001450", "HP:0012275", "HP:0010984", "HP:0010983", "HP:0010982"]:
    ...
    def __get_facial_symptoms(orpha_id, symptoms_max_number=int):
        x = df_genes_to_phenotype.loc[
            (df_genes_to_phenotype['disease_id'] == orpha_id) & (df_genes_to_phenotype['is_facial'] == 1)]
        result = []
    ...
    def get_symptoms(field_name, omim_id=None, orpha_id=None, symptoms_max_number=5):
        if omim_id is not None:
            return headfake.field.OperationField(name=field_name, operator=None, first_value=omim_id,
                                                 second_value=symptoms_max_number,
                                                 operator_fn=Diagnosis.get_symptoms_by_omim_id)
        elif orpha_id is not None:
            return headfake.field.OperationField(name=field_name, operator=None, first_value=orpha_id,
                                                 second_value=symptoms_max_number,
                                                 operator_fn=Diagnosis.get_symptoms_by_orphanet_id)
        else:
    ...
    def get_facial_symptoms(field_name, orpha_id, symptoms_max_number=5):
        return headfake.field.OperationField(name=field_name, operator=None, first_value=orpha_id,
                                             second_value=symptoms_max_number,
                                             operator_fn=Diagnosis.__get_facial_symptoms)

    @staticmethod
    def get_facial_HPO(field_name, symptoms_list):
        return headfake.field.OperationField(name=field_name, operator=None, first_value=symptoms_list,
                                             second_value=None, operator_fn=Diagnosis.__get_facial_hpo)
