...
df_measures = pd.read_csv(get_ground_data("metabolic_measures_normal_ranges.csv"), sep=";")
...
class MetabolicTest:
    ...
    def generate_sample_date(field_name, start_date):
        return headfake.field.OperationField(name=field_name, operator=None, first_value=start_date, second_value=None,
                                             operator_fn=MetabolicTest.get_sample_date)

    @staticmethod
    def too_young_sample(field_name, date_of_birth, date_sample_collected):
        return headfake.field.OperationField(name=field_name, operator=None, first_value=date_of_birth,
                                             second_value=date_sample_collected, operator_fn=MetabolicTest.is_too_young)
