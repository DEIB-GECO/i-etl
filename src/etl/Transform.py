...
class Transform:
    ...
    def create_hospital(self, hospital_name: str) -> None:
        ...
        write_in_file(resource_list=copy.deepcopy(self._hospitals), current_working_dir=self._execution.working_dir_current, table_name=TableNames.HOSPITAL.value, count=1)
    ...
    def create_examinations(self) -> None:
        ...
        count = 1
        index_for_column_name = self._metadata.columns.get_loc(MetadataColumns.COLUMN_NAME.value)
        log.debug(f"The index of {MetadataColumns.COLUMN_NAME.value} is {index_for_column_name}")
        for row_index, row in self._metadata.iterrows():
            ...
            if column_name not in NO_EXAMINATION_COLUMNS:
                ...
                if cc is not None:
                    ...
                    category = self.determine_examination_category(column_name=column_name)
                    ...
                    if len(self._examinations) >= BATCH_SIZE:
                        write_in_file(resource_list=self._examinations, current_working_dir=self._execution.working_dir_current, table_name=TableNames.EXAMINATION.value, count=count)
                        self._examinations = []
                        count = count + 1
                else:
        ...
        for index, row in self._data.iterrows():
            ...
            for column_name, value in row.items():
                ...
                if value is None or value == "" or not is_not_nan(value):
                    ...
                    if column_name in self._mapping_column_to_examination_id:
                        ...
                        if len(self._examination_records) >= BATCH_SIZE:
                            write_in_file(resource_list=self._examination_records, current_working_dir=self._execution.working_dir_current, table_name=TableNames.EXAMINATION_RECORD.value, count=count)
        ...
        for index, row in self._data.iterrows():
            ...
            if patient_id not in created_patient_ids:
                ...
                if len(self._patients) >= BATCH_SIZE:
                    write_in_file(resource_list=self._patients, current_working_dir=self._execution.working_dir_current, table_name=TableNames.PATIENT.value, count=count)  # this will save the data if it has reached BATCH_SIZE
                    self._patients = []
                    count = count + 1
                    # no need to load Patient instances because they are referenced using their ID,
                    # which was provided by the hospital (thus is known by the dataset)
        write_in_file(resource_list=self._patients, current_working_dir=self._execution.working_dir_current, table_name=TableNames.PATIENT.value, count=count)

    def create_codeable_concept_from_column(self, column_name: str) -> CodeableConcept | None:
        rows = self._metadata.loc[self._metadata[MetadataColumns.COLUMN_NAME.value] == column_name]
        log.debug(rows)
        if len(rows) == 1:
            row = rows.iloc[0]
            cc = CodeableConcept()
                ...
                if value == mapped_value:
                    ...
                    for key, val in mapping.items():
                        ...
                        if key != 'value' and key != 'explanation':
                            ...
                            cc.add_coding(coding=Coding(system=system, code=val, display=display))
    ...
    def examination_records(self):
        return self._examination_records
    ...
    @property
    def mapping_column_to_examination_id(self) -> dict:
        return self._mapping_column_to_examination_id

    @property
    def mapping_disease_to_disease_id(self) -> dict:
        return self._mapping_disease_to_disease_id

    @property
    def mapping_hospital_to_hospital_id(self) -> dict:
        return self._mapping_hospital_to_hospital_id
