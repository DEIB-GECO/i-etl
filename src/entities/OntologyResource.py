    ...
    def __post_init__(self):
        ...
        if len(self.system) == 0 or len(self.code) == 0:
            ...
            if self.label is not None and self.label != "":
                log.warn(f"Creating an OntologyResource with a label only (label={self.label}).")
            else:
                log.warn("Could not create an OntologyResource with no ontology system and/or code.")
        else:
