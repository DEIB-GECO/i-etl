...
class TestDatabase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        args = {
            ...
            Execution.DB_UPSERT_POLICY_KEY: UpsertPolicy.DO_NOTHING.value
        }
    ...
    def test_drop(self):
        ...
        database.db[TableNames.TEST.value].insert_one(document={"id": "1", "name": "Alice Doe"})
    ...
    def test_insert_empty_tuple(self):
        ...
        database.insert_one_tuple(table_name=TableNames.TEST.value, one_tuple=my_tuple)
        docs = [doc for doc in database.db[TableNames.TEST.value].find({})]
    ...
    def test_insert_many_tuples(self):
        ...
        database.insert_many_tuples(table_name=TableNames.TEST.value, tuples=my_tuples)
        docs = [doc for doc in database.db[TableNames.TEST.value].find({}).sort({"id": 1})]
    ...
    def test_upsert_one_tuple_single_key(self):
        ...
        my_original_tuple = copy.deepcopy(my_tuple)
        database.upsert_one_tuple(table_name=TableNames.TEST.value, unique_variables=["name"], one_tuple=my_tuple)
        ...
        database.upsert_one_tuple(table_name=TableNames.TEST.value, unique_variables=["name"], one_tuple=my_tuple_age)
        docs = [doc for doc in database.db[TableNames.TEST.value].find({})]
        assert len(docs) == 1, wrong_number_of_docs(1)
        ...
        args = {
            Execution.DB_UPSERT_POLICY_KEY: UpsertPolicy.REPLACE.value,
            Execution.DB_DROP_KEY: False
        }
        TestDatabase.execution.set_up(args_as_dict=args, setup_data_files=False)
        database = Database(execution=TestDatabase.execution)
        ...
        database.upsert_one_tuple(table_name=TableNames.TEST.value, unique_variables=["name"], one_tuple=my_new_tuple)
        docs = [doc for doc in database.db[TableNames.TEST.value].find({}).sort({"name": 1})]
    ...
    def test_upsert_one_tuple_unknown_key(self):
        ...
        with pytest.raises(KeyError):
            ...
            database.upsert_one_tuple(table_name=TableNames.TEST.value, unique_variables=["city"], one_tuple=my_tuple)

    def test_upsert_one_tuple_multi_key(self):
        database = Database(execution=TestDatabase.execution)
        my_tuple = {"name": "Nelly", "age": 26}
        my_original_tuple = copy.deepcopy(my_tuple)
        database.upsert_one_tuple(table_name=TableNames.TEST.value, unique_variables=["name", "age"], one_tuple=my_tuple)
        ...
        my_tuple_age = {"name": "Nelly", "age": 27, "city": "Lyon"}  # same as my_tuple but with a different age and a new field
        my_original_tuple_age = copy.deepcopy(my_tuple_age)
        database.upsert_one_tuple(table_name=TableNames.TEST.value, unique_variables=["name", "age"], one_tuple=my_tuple_age)
        docs = [doc for doc in database.db[TableNames.TEST.value].find({}).sort({"name": 1, "age": 1})]
        ...
        database.upsert_one_tuple(table_name=TableNames.TEST.value, unique_variables=["name", "age"], one_tuple=my_new_tuple)
        docs = [doc for doc in database.db[TableNames.TEST.value].find({}).sort({"name": 1, "age": 1})]
        ...
        my_new_tuple = {"name": "Nelly", "age": 26, "city": "Lyon"}
        my_original_new_tuple = copy.deepcopy(my_new_tuple)
        database.upsert_one_tuple(table_name=TableNames.TEST.value, unique_variables=["name", "age"],
    ...
    def test_upsert_one_batch_of_tuples_single_key(self):
        ...
        database.upsert_one_batch_of_tuples(table_name=TableNames.TEST.value, unique_variables=["name"], the_batch=my_batch)
        docs = [doc for doc in database.db[TableNames.TEST.value].find({})]
        assert len(docs) == 2, wrong_number_of_docs(2)
        ...
        database.upsert_one_batch_of_tuples(table_name=TableNames.TEST.value, unique_variables=["name"], the_batch=my_batch_2)
        ...
        database.upsert_one_batch_of_tuples(table_name=TableNames.TEST.value, unique_variables=["name"], the_batch=my_batch_3)
    ...
    def test_upsert_one_batch_of_tuples_multi_key(self):
        ...
        database.upsert_one_batch_of_tuples(table_name=TableNames.TEST.value, unique_variables=["name", "age"], the_batch=my_tuples)
        ...
        database.upsert_one_batch_of_tuples(table_name=TableNames.TEST.value, unique_variables=["name", "age"], the_batch=my_original_tuples)
        docs = [doc for doc in database.db[TableNames.TEST.value].find({})]
        ...
        database.upsert_one_batch_of_tuples(table_name=TableNames.TEST.value, unique_variables=["name", "age"], the_batch=my_tuples_age)
        docs = [doc for doc in database.db[TableNames.TEST.value].find({}).sort({"name": 1, "age": 1})]
        ...
        database.upsert_one_batch_of_tuples(table_name=TableNames.TEST.value, unique_variables=["name", "age"], the_batch=my_tuples_age_2)
        docs = [doc for doc in database.db[TableNames.TEST.value].find({}).sort({"name": 1, "age": 1})]
    ...
    def test_upsert_one_batch_of_tuples_unknown_key(self):
        ...
        used_unique_variables = database.upsert_one_batch_of_tuples(table_name=TableNames.TEST.value, unique_variables=["city"], the_batch=my_tuples)
        ...
        used_unique_variables = database.upsert_one_batch_of_tuples(table_name=TableNames.TEST.value, unique_variables=["name", "city"], the_batch=my_tuples)
    ...
    def test_write_in_file(self):
        ...
        write_in_file(resource_list=my_tuples, current_working_dir=self.execution.working_dir_current, table_name=TableNames.TEST.value, count=1)
        filepath = os.path.join(TestDatabase.execution.working_dir_current, TableNames.TEST.value+"1.json")
    ...
    def test_write_in_file_no_resource(self):
        database = Database(TestDatabase.execution)
        my_tuples = []
        write_in_file(resource_list=my_tuples, current_working_dir=self.execution.working_dir_current, table_name=TableNames.TEST.value, count=2)
        filepath = os.path.join(TestDatabase.execution.working_dir_current, TableNames.TEST.value + "2.json")
    ...
    def test_load_json_in_table(self):
        ...
        database.load_json_in_table(table_name=TableNames.TEST.value, unique_variables=["name", "age"])
    ...
    def test_find_operation_1(self):
        ...
        my_original_tuples = copy.deepcopy(my_tuples)
        database.db[TableNames.TEST.value].insert_many(documents=my_tuples)

        # 1. no filter, no projection
        docs = [doc for doc in database.find_operation(table_name=TableNames.TEST.value, filter_dict={}, projection={}).sort({"name": 1})]
        assert len(docs) == len(my_original_tuples), wrong_number_of_docs(len(my_original_tuples))
        for i in range(len(my_original_tuples)):
            compare_tuples(original_tuple=my_original_tuples[i], inserted_tuple=docs[i])

        # 2. a single filter, no projection
        docs = [doc for doc in database.find_operation(table_name=TableNames.TEST.value, filter_dict={"city": "Lyon"}, projection={})]
        ...
        docs = [doc for doc in database.find_operation(table_name=TableNames.TEST.value, filter_dict={"name": "Nelly", "city": "Lyon"}, projection={})]
        ...
        docs = [doc for doc in database.find_operation(table_name=TableNames.TEST.value, filter_dict={}, projection={"name": 1}).sort({"name": 1})]
        ...
        docs = [doc for doc in database.find_operation(table_name=TableNames.TEST.value, filter_dict={"city": "Lyon"}, projection={"name": 1})]
        ...
        docs = [doc for doc in database.find_operation(table_name=TableNames.TEST.value, filter_dict={"name": "Nelly", "city": "Lyon"}, projection={"name": 1})]
        ...
        docs = [doc for doc in database.find_operation(table_name=TableNames.TEST.value, filter_dict={}, projection={"name": 1, "job": 1}).sort({"name": 1, "job": 1})]
        ...
        docs = [doc for doc in database.find_operation(table_name=TableNames.TEST.value, filter_dict={"city": "Lyon"}, projection={"name": 1, "job": 1})]
        expected_docs = [
        ...
        docs = [doc for doc in database.find_operation(table_name=TableNames.TEST.value, filter_dict={"name": "Nelly", "city": "Lyon"}, projection={"name": 1, "job": 1})]
        expected_docs = [
            {"name": my_original_tuples[3]["name"], "job": my_original_tuples[1]["job"]},
            {"name": my_original_tuples[4]["name"], "job": my_original_tuples[2]["job"]}
        ]
    ...
    def test_count_documents(self):
        ...
        current_count = database.count_documents(table_name=TableNames.TEST.value, filter_dict={})
        ...
        current_count = database.count_documents(table_name=TableNames.TEST.value, filter_dict={"from": "France"})
        ...
        current_count = database.count_documents(table_name=TableNames.TEST.value, filter_dict={"from": "France", "name": "Nelly"})
        assert current_count == len(expected_docs), wrong_number_of_docs(len(expected_docs))
    ...
    def test_get_min_or_max_value(self):
        ...
        database.db[TableNames.TEST.value].insert_many(documents=my_tuples)
        min_value = database.get_min_or_max_value(table_name=TableNames.TEST.value, field="value", sort_order=1)
        max_value = database.get_min_or_max_value(table_name=TableNames.TEST.value, field="value", sort_order=-1)
